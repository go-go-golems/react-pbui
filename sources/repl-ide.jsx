...
========================================================================= */
function CodeEditor({ value, onChange, onRun, bindings, minRows, placeholder }) {
  const rows = minRows || 3;
  const taRef = useRef(null);
  const backdropRef = useRef(null);
  const [cursor, setCursor] = useState(value.length);
  const lines = value.length ? value.split("\n") : [""];
  const visualRows = Math.max(rows, lines.length);
  ...
  const syncScroll = (e) => { if (backdropRef.current) backdropRef.current.scrollLeft = e.target.scrollLeft; };
    ...
    <div>
      <div style={{ position: "relative", border: `1px solid ${C.ink}`, background: "#fff", display: "flex" }}>
        <div style={{
          ...
          lineHeight: LINE_H + "px", color: C.line, minWidth: 32,
        }}>
        ...
        <div style={{ position: "relative", flex: 1, minWidth: 0 }}>
          <pre
            ref={backdropRef}
            aria-hidden="true"
            style={{ ...sharedFont, margin: 0, overflowX: "auto", overflowY: "hidden", pointerEvents: "none", color: C.ink, minHeight: visualRows * LINE_H + PAD_V * 2 }}
          >
          ...
          <textarea
            ...
            style={{
              ...sharedFont, position: "absolute", inset: 0, width: "100%", height: "100%",
...
========================================================================= */
const TABS = ["execution", "static", "rewrite", "runtime", "provenance"];
...
function CellCard({ cell, bindings, onJump, sessionId, backend }) {
  const [tab, setTab] = useState("execution");
  const ok = cell.execution.status === "ok";
  return (
    <div id={"cell-" + cell.id} style={{ border: `1px solid ${C.ink}`, marginBottom: 14, background: "#fff" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "6px 10px", borderBottom: `1px solid ${C.ink}`, background: C.paperDim }}>
        <span style={{ fontFamily: FONT_MONO, fontSize: 11, color: C.ink }}>#{cell.id}</span>
        <span style={{ fontFamily: FONT_UI, fontSize: 10, letterSpacing: "0.05em", textTransform: "uppercase", color: ok ? C.olive : C.burnt }}>{ok ? "ok" : "error"}</span>
        <span style={{ fontFamily: FONT_MONO, fontSize: 11, color: C.line }}>{cell.execution.durationMs}ms</span>
        <span style={{ marginLeft: "auto", fontFamily: FONT_MONO, fontSize: 10, color: C.line }}>{new Date(cell.createdAt).toLocaleTimeString()}</span>
      </div>
      <div style={{ padding: "8px 10px", borderBottom: `1px solid ${C.line}` }}>
        <pre style={{ margin: 0, fontFamily: FONT_MONO, fontSize: 13, lineHeight: LINE_H + "px", whiteSpace: "pre-wrap", wordBreak: "break-word" }}>
          <CodeTokens code={cell.source} bindings={bindings} onJump={onJump} />
        </pre>
      </div>
      <div style={{ display: "flex", gap: 0, padding: "6px 10px 0", flexWrap: "wrap" }}>
        {TABS.map((t) => (
          <button key={t} className={"retro-tab" + (tab === t ? " active" : "")} onClick={() => setTab(t)}>{TAB_LABELS[t]}</button>
        ))}
      </div>
      <div style={{ border: `1px solid ${C.ink}`, borderTop: "none", padding: 10, background: "#fff" }}>
        {tab === "execution" && <ExecutionTab report={cell.execution} sessionId={sessionId} backend={backend} />}
        {tab === "static" && <StaticTab report={cell.static} />}
        {tab === "rewrite" && <RewriteTab report={cell.rewrite} />}
        {tab === "runtime" && <RuntimeTab report={cell.runtime} />}
        {tab === "provenance" && <ProvenanceTab items={cell.provenance} />}
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  const handleRun = useCallback(async () => {
    if (!editorValue.trim() || running || !activeId) return;
    setRunning(true);
    setRunError("");
    try {
      const { session, cell } = await backend.evaluate(activeId, editorValue);
  ...
  }, [backend, activeId, editorValue, running]);
    ...
    <div className="retro-root" style={{ color: C.ink, background: C.paper, padding: 16, maxWidth: 1180, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", gap: 16, marginTop: 16, alignItems: "flex-start", flexWrap: "wrap" }}>
        ...
        <div style={{ flex: "3 1 460px", minWidth: 300 }}>
          <Panel title="Editor">
            <CodeEditor value={editorValue} onChange={setEditorValue} onRun={handleRun} bindings={bindingsMap} minRows={3} placeholder={"type javascript, then run (\u2318\u21b5)"} />
            <div style={{ display: "flex", gap: 8, marginTop: 10, alignItems: "center", flexWrap: "wrap" }}>
              <button className="retro-btn inverted" onClick={handleRun} disabled={running || !editorValue.trim() || !ready}>
                {running ? "running\u2026" : "run \u25b6"}
              </button>
          ...
          <div style={{ marginTop: 16 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8, flexWrap: "wrap" }}>
              <span style={{ fontFamily: FONT_UI, fontSize: 11, letterSpacing: "0.08em", textTransform: "uppercase" }}>History</span>
              <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                {historyChips.map((h) => (
                  <button
                    key={h.cellId}
                    className="retro-chip"
                    style={{ color: h.status === "ok" ? C.olive : C.burnt }}
                    onClick={() => onJump(h.cellId)}
                  >
                    {h.cellId}
                  </button>
                ))}
            ...
            <div style={{ maxHeight: 560, overflowY: "auto", paddingRight: 4 }}>
              ...
              {detail && detail.cells.map((cell) => (
                <CellCard key={cell.id} cell={cell} bindings={bindingsMap} onJump={onJump} sessionId={activeId} backend={backend} />
              ))}
            </div>
          </div>
        </div>
